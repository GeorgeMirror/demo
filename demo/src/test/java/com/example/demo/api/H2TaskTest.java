...
public class H2TaskTest {
    ...
    @Test
    public void delete_mistake(){
        taskService.modeH2();
        HttpClient httpClient = HttpClients.createDefault();
        HttpDelete httpDelete = new HttpDelete(URL+"/1998");
        ...
        HttpEntity entity = response.getEntity();
        try {
            Assert.assertTrue(EntityUtils.toString(entity).equals("Нет такого id в базе данных"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void update_mistake(){
        ...
        params.add(new BasicNameValuePair("description", "TEST DESCRIPTION"));
        params.add(new BasicNameValuePair("date", LocalDateTime.now().toString()));
        params.add(new BasicNameValuePair("id", "199"));
        try {
            httpPatch.setEntity(new UrlEncodedFormEntity(params, "UTF-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        ...
        HttpEntity entity = response.getEntity();
        try {
            Assert.assertTrue(EntityUtils.toString(entity).equals("Нет такого id в базе данных. Воспользуйтесь созданием."));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
